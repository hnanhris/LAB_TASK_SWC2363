...
if ($_SERVER ['REQUEST_METHOD']== 'POST') {
    $error = array (); //initialize a error array
...
if (empty($_POST ['Specialization'])) {
    $error [] = 'You forgot to enter you specialization.';
}
else {
...
if (empty($_POST ['Password'])) {
    $error [] = 'You forgot to enter you Password.';
}
else {
    $p = mysqli_real_escape_string ($connect, trim ($_POST [ 'Password']));
}

    //registerthe user in the database
    //make the query:
...
<p><label class ="label" for ="LastName"> First Name : *</label>
...
<input id = "Specialization" type="text" name="Specialization" size="30" maxlength="30"
value="<?php if (isset($_POST['Specialization'])) echo $_POST ['Specialization']; ?> " />
</p>

<p><label class ="label" for ="Password"> Password : *</label>
<input id = "Password" type="text" name="Password" size="30" maxlength="30"
